...
object ToysrusParser extends Parser {
  ...
  override def getSku(doc: Document): Option[String] = {
    ParseUtils.tagParser(doc, "p", "class", "skuText", Text())
  }

  override def getPrice(doc: Document): Option[String] = {
    ParseUtils.tagParser(doc, "span", "class", "adjusted ourPrice", Text())

  }

  override def getImageUrl(doc: Document): Option[String] = {

    ParseUtils.tagParser(doc, "meta", "property", "og:image", Text())
